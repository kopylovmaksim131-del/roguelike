#include "CreepSpawner.h"

namespace RoguelikeGame
{
    void CreepSpawner::SpawnCreeps(EnemyType type, int count, XYZEngine::GameObject* player, std::vector<std::vector<bool>>& availablePos)
    {
        for (int i = 0; i < count; i++)
        {
            std::vector<std::pair<int, int>> freeCells;
            for (int y = 0; y < availablePos.size(); ++y)
            {
                for (int x = 0; x < availablePos[y].size(); ++x)
                {
                    if (availablePos[y][x])
                    {
                        freeCells.push_back({ x, y });
                    }
                }
            }

            if (freeCells.empty()) break;  

            int index = std::rand() % freeCells.size();
            std::pair<int, int> cell = freeCells[index];
            int x = cell.first;
            int y = cell.second;
            availablePos[y][x] = false;

            XYZEngine::Vector2Df position{ x * 128.f, y * 128.f };
            factory->Create(type, position, player);
        }
    }

}
